...
| Purpose | Placeholder used below | GitHub visibility |
|---|---|---|
| Sync tool and workflow skill | `<skills-sync-public>` | public |
| Tim's personal skills hub | `<skills-hub-tim-private>` | private |
| Cathay/team skills hub | `<skills-hub-cathay>` | private |
| Legacy source/migration repo | `<skills-sync-legacy>` | private/archive only |

These are wherever you cloned each repo **on this machine** — the clone path
is not synced and can differ per device (Windows vs macOS vs a VM). Check
this machine's `targets.local.yaml` or your shell history if you don't
remember it; don't assume another device's path matches this one.
...
python bin\skills-sync --hub <skills-hub-tim-private> status
python bin\skills-sync --hub <skills-hub-cathay> status
```
...
- Use `--hub <skills-hub-tim-private>` for Tim's personal skills.
- Use `--hub <skills-hub-cathay>` for Cathay/team skills.
...
```powershell
cd <skills-sync-public>
git -C <skills-hub-tim-private> status -sb
...
```powershell
cd <skills-sync-public>
git -C <skills-hub-cathay> status -sb
python bin\skills-sync --hub <skills-hub-cathay> check
python bin\skills-sync --hub <skills-hub-cathay> status
```
...
from a mounted Windows path. If you want Hermes to read shared skills
...
cd <skills-sync-public>
python bin\skills-sync --hub <skills-hub-tim-private> check
python bin\skills-sync --hub <skills-hub-tim-private> status --profile opencode-user
python bin\skills-sync --hub <skills-hub-tim-private> install --profile opencode-user
```
...
cd <skills-sync-public>
python bin\skills-sync --hub <skills-hub-tim-private> check
python bin\skills-sync --hub <skills-hub-tim-private> install --profile antigravity-user
python bin\skills-sync --hub <skills-hub-tim-private> status --profile antigravity-user
```
...
```powershell
python bin\skills-sync --hub <skills-hub-tim-private> install --profile antigravity-cli
```
...
cd <skills-sync-public>
python bin\skills-sync --hub <skills-hub-tim-private> check
python bin\skills-sync --hub <skills-hub-tim-private> install
python bin\skills-sync --hub <skills-hub-tim-private> status
```
...
cd <skills-sync-public>
python bin\skills-sync --hub <skills-hub-cathay> check
python bin\skills-sync --hub <skills-hub-cathay> install
python bin\skills-sync --hub <skills-hub-cathay> status
```
...
```powershell
python bin\skills-sync --hub <skills-hub-tim-private> import codex/obsidian-search
python bin\skills-sync --hub <skills-hub-tim-private> import claude-code/obsidian-search
python bin\skills-sync --hub <skills-hub-tim-private> import cowork-vault/obsidian-search
python bin\skills-sync --hub <skills-hub-cathay> import claude-user/<skill-id>
```
